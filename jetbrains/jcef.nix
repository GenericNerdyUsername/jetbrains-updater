...
in stdenv.mkDerivation rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "jcef";
    inherit rev;
    hash = "sha256-pCLo8aUY3Dp3zKQ/M6itohmHt9WL7bWuilWwLKeO0M4=";
  };
  cef-bin = let
    fileName = "cef_binary_104.4.26+g4180781+chromium-104.0.5112.102_linux64";
    urlName = builtins.replaceStrings ["+"] ["%2B"] fileName;
  in fetchzip rec {
    name = fileName + "_minimal";
    # We have to lie about the name because jetbrain's fork expects a minimal build but the debug variant needs the full one
    url = "https://cef-builds.spotifycdn.com/${urlName}.tar.bz2";
    hash = "sha256-/HxhVEH8zXfu26/JnoaJpmTv4pe9n3whA6G3pPVKqTo=";
  }; 
  clang-fmt = fetchurl {
